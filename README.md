...
当前开发候选版新增两条本地优先链路：Connect 可把本人 Telegram/飞书私聊路由到当前 Claude Science 工作区并回传文本或受管 artifact 图片；Subagent 可让沙盒提交脱敏请求，经本地人工批准后交给 Claude Code plan 会话处理，并通过稳定 outbox 回读结果。
...
公开稳定版仍以 GitHub Latest Release 为准。候选包完成真人 Telegram 文本/图片复验并从干净提交重建前，不应替代稳定版。
