...
2. **三层订阅角色表**：现有 API Key 列表作为订阅表，新增决策、视觉、日常三行静态映射。内部继续使用旧 `default / vision / fast` ID，旧映射自动迁移到方案一。
3. **模型列表与手动映射**：添加供应商时可读取真实模型列表，启动器先给出三层建议，用户再像 CC Switch 一样用下拉框逐层确认；不会仅凭模型名宣称某个模型一定支持视觉。
4. **两个真实运行模式**：`API 接入` 一次启用一条订阅；`聚合模式` 同时启用三个模型槽，并可保存、切换方案一和方案二。
5. **整表切换是单事务**：保存聚合方案会一次解密三条本地密钥并写入同一个 Bridge 路由表；Bridge-only 启动/重启、revision 校验和 Windows 设置提交保持在同一事务内，任一步失败都会回滚。
