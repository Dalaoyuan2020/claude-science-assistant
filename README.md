...
- DeepSeek、LongCat、MiniMax、OpenCode Go 兼容修正；DeepSeek 原生接口会将 Claude Science 的 `thinking.type=auto` 转换为上游支持的 `adaptive`。
- Bridge 接管会验证 `source_path` 和配置版本，避免旧解压目录继续提供服务却被误判为新版。
- 首页提供可折叠的 2 行 × 3 列六项状态，并显示 WSL 虚拟磁盘位置与剩余空间。
