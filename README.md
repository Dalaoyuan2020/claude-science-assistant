...
1. **六状态界面与存储体检**：首页改为可折叠的 2×3 六状态卡；“WSL 存储”显示 VHDX 的 Windows 位置、宿主盘余量、Linux 根分区余量和 VHDX 大小，并在空间不足、文件系统只读或 inode 紧张时告警/阻断危险重启。
...
> 存储迁移边界：v0.1.3 r2 已上线位置检测、C 盘专用预警、容量告警和危险操作阻断；“辅助迁移按钮/生成本机迁移 Prompt”尚未进入 EXE。迁移执行仍是独立高风险流程，当前不会自动 export/import、unregister、移动或重建 WSL。
