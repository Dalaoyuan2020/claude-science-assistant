...
- 不再把 Claude Science CLI 自身的裸退出码 `2` 误报为“受管运行时入口缺失”：入口缺失只认启动器生成的明确 sentinel；`1/2/4` 切换态均做两次有界退避，失败时立即保留真实诊断，PID/端口由独立的周期或手动刷新更新。用户操作开始前已在途的慢速后台刷新会自动作废，不能再清除新诊断或回写旧 PID；Windows 看门狗预算也覆盖生命周期锁与完整重试窗口。
- “沙盒 / API 出口”深度质检不再阻塞或否决本地 Claude Science 登录：受管 Bridge 与 3/3 本地 HTTP/SOCKS 拓扑仍用于系统健康分级，匿名 PyPI canary 则只是独立黄/绿质量信号；两者都不会锁住本地登录按钮。深检命令只回写有效的网络质量字段，WSL/DrvFS 体检失败时不会再用降级占位值清空已验证的 Claude PID/端口状态。登录入口只做轻量 WSL 预检，并实时要求 8765/8766 由同一个受管 daemon 持有、可执行文件/参数/start ticks 一致且线程不在不可中断 I/O，再调用该已验证进程对应的 CLI；整段校验脚本经 Bash 标准输入传入，避免 `wsl.exe` 外层命令行提前展开函数与 `$()`。失败会立即显示，不会串行追加一次可能耗时数分钟的全量刷新。
- 启动与快速复用路径不再自动执行多轮公网深检；“深度检测”只由用户独立触发。一次探针期间捕获的 `D/p9_client_rpc` 是带时间边界的瞬态观测，不会写入 15 分钟正式缓存；旧包遗留的 busy 缓存在同一 daemon 已恢复到 `R/S/I` 时会立即失效，不能再与实时 `do_epoll_wait` 混拼成持续故障。仅在实际检测到宽泛 DrvFS 持久写授权时才建议收窄授权。
