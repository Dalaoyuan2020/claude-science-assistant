...
- 不再把 Claude Science CLI 自身的裸退出码 `2` 误报为“受管运行时入口缺失”：入口缺失只认启动器生成的明确 sentinel；`1/2/4` 切换态均做两次有界退避，失败后刷新 PID/端口状态并保留真实诊断。用户操作开始前已在途的慢速后台刷新会自动作废，不能再清除新诊断或回写旧 PID；Windows 看门狗预算也覆盖生命周期锁与完整重试窗口。
