...
> r2 会在 VHDX 位于 C 盘时直接标记存储警告，但不会自动迁移 WSL。“辅助迁移”按钮、本机迁移 Prompt，以及实际 Move/export/import/unregister，均未进入 v0.1.3 EXE。
...
CSA v0.1.3 只提供位置检测、容量/可写性告警和危险操作阻断，不会自动迁移、注销、压缩或重建用户的 WSL 发行版。C 盘定向辅助迁移入口仍需在后续版本完成只读方案生成、兼容矩阵和异机验收后再发布。
