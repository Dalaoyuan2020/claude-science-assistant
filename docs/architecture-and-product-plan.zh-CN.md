...
启动器首页不平铺所有 Provider，也不逐个测试所有 Key。首页按添加顺序展示 API Key 条目，只突出一条当前 Key，并提供“添加 API Key”入口。用户点击添加入口后，服务商模板按“官方直连优先、聚合平台其次、第三方中转最后”排列。这里的“默认”表示内置入口与推荐顺序，不表示替用户自动登录、自动充值或自动启用某个 Provider。
...
- 服务商模板只在“添加 API Key”流程中展示，按顺序纵向排列；首页按添加顺序展示用户已经保存的 API Key 条目。
...
```text
你正在维护 claude-science-api-bridge。先阅读 AGENTS.md、
