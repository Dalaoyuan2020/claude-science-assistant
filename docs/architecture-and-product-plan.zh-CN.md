...
v0.1.6 同时把 MCP 改为真正按需加载。受管二进制副本禁用启动阶段的 `QT9` bundled-server 预热；catalog snapshot 调用 `_loadBundledServer(serverName, false)` 时固定 no-op，不会因为枚举目录或生成快照启动 24 个 MCP；buildApp 的 custom-MCP 元数据预热也提前返回。Fastify `onReady` 的 `NYz()` 则完整保留，因为它负责默认 Python/R 与 BYOC skeleton 的排队、创建、状态广播和失败重试。为防这条首装链经 `wrapCondaCommand()` 递归扫描无关 DrvFS 授权，只有 Conda 管理 wrapper 改为传入 vendor 已初始化的显式空 Git 快照 `XV9`；不能传 `null`/`undefined`，后者会触发 `JQG` 的递归 fallback。connector/MCP 进程仍只在实际使用时加载，默认计算环境的首装语义不变。

另一条独立启动链是 sandbox Git safety warm scan。Claude Science 0.1.25 会在 daemon build 阶段递归扫描所有持久 `rw:` host grant；若存在 `rw:/mnt/e/Downloads` 这类宽泛 DrvFS 授权，深度为 4 的目录遍历可能让 Bun 主线程进入 `D/p9_client_rpc`。v0.1.6 把唯一的启动期 `warmGitScan()` 调用改为 no-op，并让无 frame/user workspace 写入面的 Conda 管理 wrapper 使用显式空快照；仍保留持久授权 hydrate、`pinBaseRootsAfterGrantProjection()`、网络 deny、`warmGitScan()` 方法本体及真实 analysis/MCP/internal-env wrapper 的 `_ensureGitScan()`。所以 daemon 与 Python/R 首装不再递归授权目录，首次真实沙盒命令仍必须完成同一 Git 安全扫描后才能执行，后续新增授权也仍会触发 warm scan。

启动器只读解析授权字符串而不遍历对应路径，并按 0.1.25 的 load 语义兼容现代 `approvalGrants` 对象、旧数组和未迁移且未过期的 `hostGrants`；它报告所有有效 DrvFS RW grant，同时单独标记 drive root、Downloads、Documents、Desktop 等标准宽泛项。检测到风险时状态为“需要修复”，“修复并重启”会在 daemon 安全停止后自动执行 `--convert-drvfs-rw-to-ro`；也可手工用 `--apply` 仅删除标准宽泛项。两种模式都先写入内容哈希命名的 0600 原始备份，再通过同目录临时文件、CAS 检查、`fsync` 与原子替换提交。读取能力、ext4 RW grant 和其他偏好保持不变。
