...
首次打开启动器时，CSA 会把 Claude Science 作为产品核心服务优先唤起一次。事务内部仍需先验证本地 Bridge `9876`，再启动把 `ANTHROPIC_BASE_URL` 指向该 Bridge 的 Claude Science；这是依赖顺序，不代表 Bridge 是用户需要手工先启动的主服务。30 秒状态刷新不会重复启动。若发现旧 Windows Bridge、未知端口占用、单端口拓扑、磁盘/WSL 阻塞或运行时不完整，CSA 不会自动重启或终止进程，而会保留诊断与显式迁移/手动入口。
...
点击“打开 Claude Science”。如果服务刚完成重启，启动器会先等待受管生命周期切换和本地登录控制通道稳定，再由 Rust 后端把一次性 loopback 登录地址直接交给默认浏览器；地址中的 nonce 不会显示在启动器界面或诊断信息中。日常使用不需要重新安装，也不需要打开终端。
