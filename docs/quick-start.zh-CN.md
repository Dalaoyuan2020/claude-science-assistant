...
如果“WSL 存储”显示在 C 盘，v0.1.3 r2 会直接显示存储警告，但不会自动搬迁发行版。当前也没有可直接执行的“一键迁移”按钮；不要手工移动 `ext4.vhdx`。需要迁移时，先让 Codex 只读检查 WSL 版本、发行版 GUID、默认用户、systemd、目标盘和备份空间，再依据经过审核的迁移方案逐阶段确认。
