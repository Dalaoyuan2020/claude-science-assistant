...
> 发布编号补记（2026-08-25）：本报告记录的是当时未发布的内部 UI 候选，不是一个已发布版本。用户随后明确要求公开版使用 **v0.1.7**，并在该候选上补齐第三套深色外观、内置帮助、二维码、对比度和版本一致性硬门；最终公开验收另见 `CSA_v017_public_release_acceptance_20260825.md`。以下内容保留原始候选口径，避免静默改写历史。
