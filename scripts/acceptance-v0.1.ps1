...
$required = @(
  ...
  "tests\runtime_network_contract_functions_test.sh",
  "tests\runtime_lifecycle_20cycle_test.sh",
  "scripts\package-policy.ps1",
  ...
  "launcher\src\storageMigration.ts",
  "launcher\src-tauri\src\lib.rs",
  "launcher\src-tauri\src\runtime_lifecycle.rs",
...
Write-Host "- The home screen lists saved API Keys in add order; provider templates appear only inside Add Provider."
...
Write-Host "- Project-operated relay shows https://10521052.xyz/v1, is not presented as a model-vendor official API, and still requires domain confirmation."
