...
PACKAGE_DIR="${CSA_PACKAGE_DIR:-$(cd "$SCRIPT_DIR/.." && pwd)}"
...
BRIDGE_CURRENT="$CSA_STATE_ROOT/runtime/bridge/current"
BRIDGE_PROXY="$BRIDGE_CURRENT/proxy.py"
BRIDGE_STATIC="$BRIDGE_CURRENT/static"

for name in CSA_BRIDGE_RUNTIME_ID CSA_BRIDGE_VERSION CSA_BRIDGE_SOURCE_SHA256; do
  if [ -z "${!name:-}" ]; then
    echo "Managed Bridge identity is incomplete: $name is missing." >&2
    exit 2
  fi
done
if [ ! -f "$BRIDGE_PROXY" ] || [ ! -f "$BRIDGE_STATIC/dashboard.html" ]; then
  echo "Managed Bridge current pointer is incomplete: $BRIDGE_CURRENT" >&2
  exit 2
fi
...
if [ ! -f "$CONFIG_DIR/config.json" ] && [ -f "$PACKAGE_DIR/config.json" ]; then
  cp -f "$PACKAGE_DIR/config.json" "$CONFIG_DIR/config.json"
  chmod 600 "$CONFIG_DIR/config.json"
fi
python_escaped="$(unit_escape "$PYTHON_BIN")"
proxy_escaped="$(unit_escape "$BRIDGE_PROXY")"
config_dir_escaped="$(unit_escape "$CONFIG_DIR")"
static_dir_escaped="$(unit_escape "$BRIDGE_STATIC")"
runtime_id_escaped="$(unit_escape "$CSA_BRIDGE_RUNTIME_ID")"
runtime_version_escaped="$(unit_escape "$CSA_BRIDGE_VERSION")"
runtime_sha_escaped="$(unit_escape "$CSA_BRIDGE_SOURCE_SHA256")"
...
Environment="CSA_BRIDGE_RUNTIME_ID=$runtime_id_escaped"
Environment="CSA_BRIDGE_VERSION=$runtime_version_escaped"
Environment="CSA_BRIDGE_SOURCE_SHA256=$runtime_sha_escaped"
